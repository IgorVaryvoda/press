...
`--skip-existing` skips current outputs whose recorded format, quality, maximum
edge, and AVIF speed match the requested settings. A recorded output is rebuilt
when those settings change. Outputs without a matching, intact manifest record
retain the timestamp-only fallback.
