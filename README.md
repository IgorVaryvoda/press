...
press convert ~/path/to/folder --skip-existing           # only what changed
...
is not older than it, so a re-run over a large tree re-encodes only what changed. With `--json`, stdout contains one document with
