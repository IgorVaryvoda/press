//! Asset source: app icons, then everything gpui-component ships.
//!
...
//!
//! The Studio files are Hugeicons, MIT, taken from the same catalogue Studio renders
...
macro_rules! icons {
    ($($slug:literal),* $(,)?) => {
        &[$((
            concat!("icons/", $slug, ".svg"),
            include_bytes!(concat!("../assets/icons/", $slug, ".svg")).as_slice(),
        )),*]
    };
}
...
mod tests {
    ...
    fn studio_icons_load_before_the_bundled_set_and_do_not_hide_it() {
        ...
        for side in ["left", "right"] {
            assert!(
                Assets
                    .load(&format!("icons/panel-{side}-filled.svg"))
                    .expect("loads")
                    .is_some()
            );
        }
